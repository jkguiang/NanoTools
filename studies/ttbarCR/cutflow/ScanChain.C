...
void analysis(int &gen_lept_event, int &gen_elec_event_t, int &gen_mu_event_t, int &gen_one_and_one_t, int &gen_opp_ch_t, int &gen_two_b_t, int &lept_event, int &elec_event_t, int &mu_event_t, int &one_and_one_t, int &opp_ch_t, int &pass_hlt_t, int &two_btag_t, bool isData){
	...
	for (unsigned int i = 0; i < nGenPart(); i++) {
		int m_index = fabs(GenPart_genPartIdxMother().at(i));
		//if(fabs(GenPart_pdgId().at(m_index)) == 24 && (GenPart_statusFlags().at(i) & (1<<13)) == (1<<13)){
		if(fabs(GenPart_pdgId().at(m_index)) == 24){
			if(fabs(GenPart_pdgId().at(i)) == 13) {
				numGenMu++;
				gen_mu_id = i;
			}
			else if(fabs(GenPart_pdgId().at(i)) == 11) {
				numGenElec++;
				gen_el_id = i;
			}	
		}
